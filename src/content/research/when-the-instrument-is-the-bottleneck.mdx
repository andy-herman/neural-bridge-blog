...
This is the full record: ten experiments over two weeks, on a Mac Mini M4 with 24 GB of memory, with all training, generation, judging and inference on the device.
...
A teacher model writes question-answer pairs for each chunk. An early cloud teacher hit its free-tier quota within a day. Generation moved to Gemma 3 12B running locally, which removed the quota and keeps every chunk on the machine. Pairs are deduplicated, length-bounded and filtered by an LLM judge.
...
Windows line endings or malformed frontmatter cannot open the gate. Every chunk carries a provenance class, and only public text and my own writing may leave the machine or train anything.
